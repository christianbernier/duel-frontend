...
export class JoinRoomEffects {
  ...
  joinRoom$ = createEffect((): Observable<Action> => {
    return this.actions$.pipe(
      ofType(fromActions.joinRoom),
      concatMap((action) =>
        of(action).pipe(
          withLatestFrom(this.store.pipe(select(fromSelectors.getRoomUid))),
        ),
      ),
      map(([action, roomUid]): Action => {
        this.roomService.joinRoom(
          ...
          (error: Error): void =>
            this.roomFacade.updateServerError(error.message),
