...
export class RoomFacade implements IRoomFacade {
  ...
  public joinRoom(roomUid: string, name: string): void {
    this.store.dispatch(fromActions.joinRoom({
      roomUid,
      name,
    }));
  }
  ...
  public processMessage(message: object): void {
    this.store.dispatch(fromActions.processMessage({
      message,
    }));
  }

  public updateGameState(gameState: GameState): void {
    this.store.dispatch(fromActions.updateGameState({
      gameState,
    }));
  }

  public updateServerError(error: string): void {
    this.store.dispatch(fromActions.updateServerError({
      error,
    }));
  }
